...
export function VermogensavondCta() {
  return (
    <section
      ...
      className="flex flex-col gap-6 rounded-2xl bg-foreground px-5 py-6 text-background shadow-sm sm:px-8 sm:py-7 lg:flex-row lg:items-center lg:justify-between"
    >
      <div className="flex max-w-3xl flex-col gap-3">
        ...
        <div className="flex flex-col gap-2">
          ...
          <p className="max-w-2xl text-pretty text-sm leading-6 text-background/65 sm:text-base">
            Krijg helder zicht op je vertrekpunt, ontdek je blinde vlekken en zet tijdens de Vermogensavond de volgende stap naar een sterk financieel plan.
          </p>
        </div>
        <div className="flex flex-wrap gap-2" aria-label="Wat je mag verwachten">
          {['Je vertrekpunt helder', 'Inzicht in je blinde vlekken', 'Concrete volgende stappen'].map((item) => (
            <span key={item} className="rounded-full bg-background/10 px-3 py-1 text-xs font-medium text-background/80">
              {item}
            </span>
          ))}
        </div>
      ...
      <Link
        href={WORKSHOPS_URL}
        className="group inline-flex min-h-12 shrink-0 items-center justify-center gap-2 rounded-xl bg-card px-5 py-3 text-sm font-bold text-card-foreground transition-transform hover:-translate-y-0.5 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-background focus-visible:ring-offset-2 focus-visible:ring-offset-foreground"
      >
